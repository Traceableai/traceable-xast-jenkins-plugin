...
class SamplePlugin(Plugin):
    ...
    # Optional Fields
    ###############################################################################
    displayName = ""  # Display name for plugin in UI
    affectedEntityType = "CUSTOM"  # API/SERVICE/CUSTOM
    ...
    def run(self):
        ...
        # Logic to run the plugin only once per scan. Comment the following to run for each API
        if self.scan_ctx.get(self.name, False):
            return
        else:
            self.scan_ctx[self.name] = True
