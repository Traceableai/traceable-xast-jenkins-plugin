...
def sample_login_jwt_2(ctx: ScanContext, attributes: AttributeList) -> list[Assertion]:
    pass


def sample_crapi_login_posthook(ctx: ScanContext, attributes: AttributeList) -> list[Assertion]:
    res_code = int(attributes.get_one("mutated.http.response.code", 0))
    # err_code = attributes.get_one("mutated.http.response.json.error", "")
    if res_code == 401 and ctx["crapi_token"]:
        del ctx["crapi_token"]
    return []


def sample_crapi_login_prehook(ctx: ScanContext, attributes: AttributeList) -> list[Assertion]:
    url = urllib.parse.urlparse(
        attributes.get_one("original.http.request.url", ""))
    logger.info("Attempting login to CRAPI")
    if ctx.get("crapi_user", None) is None and ctx.get("crapi_token", None) is None:
        # Register:
    ...
    if ctx.get("crapi_token", None) is None:
        # Login:
        u = urllib.parse.urlunparse(
            url._replace(path="/identity/api/auth/login"))
        res = requests.post(u, json={
            "email": ctx.get("crapi_user", "juan@not-traceable.ai"),
            "password": "Traceable1!"
        }, headers={
            "content-type": "application/json"
        })
        ...
        res = res.json()
        ctx["crapi_token"] = res["token"]
        logger.info("got crapi_token: %s" % ctx["crapi_token"])
    else:
        logger.info("Already logged in to CRAPI")
    attributes.set("mutated.http.request.header.authorization",
                   "Bearer %s" % ctx["crapi_token"])
    return []


def sample_traceable_refresh_login_prehook(ctx: ScanContext, attributes: AttributeList) -> list[Assertion]:
    session = ctx.get("traceable_session", None)
...
def sample_traceable_login_prehook(ctx: ScanContext, attributes: AttributeList) -> list[Assertion]:
    session = ctx.get("traceable_session", None)
    if session is not None:
        ...
        return []
    session = ctx["traceable_session"] = requests.Session()
